...
def generate_f_matricula(n=10000, dimension_dfs=None):
    ...
    for i in tqdm(range(1, n+1), desc="Generando matrículas"):
        ...
        data.append({
            ...
            'ID_MODALIDAD_PLAN_NK': fake.lexify('?????', letters='ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789'),
            'ID_GRUPO': fake.random_int(min=1, max=1000),
            'FLG_CAMBIO_CICLO_GRADO': fake.random_element(elements=[0, 1]),
            'FLG_TRASLADO_MISMO_ESTUDIO': fake.random_element(elements=[0, 1]),
            'ID_TIPO_ESTUDIO_NK': fake.random_int(min=1, max=1000),
            'ID_PROGRAMA_MOVILIDAD_NK': id_programa_movilidad_nk,
            'SN_COORDINADOR': fake.random_element(elements=['S', 'N']),
            'ID_UNIVERSIDAD_ORIGEN': fake.random_int(min=1, max=1000),
            'SN_PROGRAMA_INTERNACIONAL': fake.random_element(elements=['S', 'N']),
            'ID_PAIS_BACH_PROCEDENCIA_NK': fake.lexify('?????', letters='ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789'),
            'ID_TIPO_ESTUDIO': fake.random_int(min=1, max=1000),
            'NUM_REGS_GRANO': fake.random_int(min=1, max=1000),
            'ID_PAIS_BACH_PROCEDENCIA': fake.random_int(min=1, max=1000),
            'FECHA_CARGA': generate_random_date(2020, 2023),
            'ID_ESTUDIO_PREVIO': fake.random_int(min=1, max=1000),
            'ID_TIPO_CENTRO': fake.random_int(min=1, max=1000),
            'SN_NUEVO_INGRESO': fake.random_element(elements=['S', 'N']),
            'ID_POBLACION_CENTRO': fake.random_int(min=1, max=1000),
            'ID_UNIVERSIDAD_ORIGEN_NK': fake.random_int(min=1, max=1000),
            'ID_DETALLE_CUPO_GENERAL_NK': random.choice(['GEN', 'DIS', 'DEP', 'M25', 'M40', 'M45', 'ESP', 'INT']),
            'ID_ASIGNATURA_NK': fake.random_int(min=1, max=1000),
            'SN_DOCTORADO_VIGENTE': fake.random_element(elements=['S', 'N']),
            'ID_DEDICACION': fake.random_int(min=1, max=1000),
            'SN_MULTIPLE_TITULACION': fake.random_element(elements=['S', 'N']),
            'ID_TITULO_PREVIO_MASTER_NK': fake.text(max_nb_chars=11).replace('\n', ' '),
            'ID_PLAN_GRUPO_MATRICULA_NK': fake.random_int(min=1, max=1000),
            'ID_CENTRO_NK': fake.random_int(min=1, max=1000),
            'ID_DEDICACION_NK': fake.random_int(min=1, max=1000),
            'ID_PLAN_ESTUDIO_ANO_DATOS': fake.random_int(min=1, max=1000),
            'FLG_MOVILIDAD_IN': fake.random_element(elements=[0, 1]),
            'FLG_ESTUDIANTE_INTERNACIONAL': fake.random_element(elements=[0, 1]),
            'SN_ESTUDIANTE_INTERNACIONAL': fake.random_element(elements=['S', 'N']),
            'ID_RANGO_NOTA_CRUE': round(fake.random.uniform(0, 10), 2),
            'FLG_COORDINADOR': fake.random_element(elements=[0, 1]),
            'ID_PLAN_GRUPO_MATRICULA': fake.random_int(min=1, max=1000),
            'FLG_EQUIVALENTE_TC_SIIU_MOV': fake.random_element(elements=[0, 1]),
            'SN_PROG_MOVILIDAD': fake.random_element(elements=['S', 'N']),
            'COD_POSTAL_CURSO': fake.lexify('?????', letters='0123456789'),
            'FLG_NUEVO_ACCESO_SUE': fake.random_element(elements=[0, 1]),
            'SN_CAMBIO_CICLO_GRADO_NUEVO': fake.random_element(elements=['S', 'N']),
            'ANYO_ACCESO_SUE': fake.random_int(min=1, max=1000),
            'ID_CENTRO_IMPARTICION_NK': fake.random_int(min=1, max=1000),
            'FLG_EQUIVALENTE_TC_SIIU': fake.random_element(elements=[0, 1]),
            'ID_EDAD_EST': fake.random_int(min=1, max=1000),
            'ID_PAIS_FAMILIAR': fake.random_int(min=1, max=1000),
            'SN_MASTER_HABILITANTE': fake.random_element(elements=['S', 'N']),
            'SN_DISCAPACIDAD': fake.random_element(elements=['S', 'N']),
            'ID_RAMA_CONOCIMIENTO': fake.random_int(min=1, max=1000),
            'SN_TRASLADO_MISMO_ESTUDIO': fake.random_element(elements=['S', 'N']),
            'ORDEN_BOE_PLAN': fake.text(max_nb_chars=16).replace('\n', ' '),
            'ID_ESTUDIO_NK': fake.random_int(min=1, max=1000),
            'COD_POSTAL_FAMILIAR': fake.text(max_nb_chars=50).replace('\n', ' '),
            'SN_MOVILIDAD_IN': fake.random_element(elements=['S', 'N']),
            'FLG_INTERUNIVERSITARIO': fake.random_element(elements=[0, 1]),
            'ID_FECHA_CARGA': int(generate_random_date(2020, 2023).replace('-', '')),
            'ID_DETALLE_CUPO_GENERAL': fake.random_int(min=1, max=1000),
            'ID_CAMPUS_CENTRO': fake.random_int(min=1, max=1000),
            'ID_ESTUDIO': fake.random_int(min=1, max=1000),
            'FLG_NUEVO_INGRESO': fake.random_element(elements=[0, 1]),
            'SN_NUEVO_ACCESO_SUE': fake.random_element(elements=['S', 'N']),
            'ID_TIPO_ACCESO_NK': fake.lexify('?????', letters='ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789'),
            'ID_PAIS_UNIVERSIDAD_ORIGEN': fake.random_int(min=1, max=1000),
            'FLG_DISCAPACIDAD': fake.random_element(elements=[0, 1]),
            'ID_TIPO_ASIGNATURA': fake.random_int(min=1, max=1000),
            'ID_PAIS_FAMILIAR_NK': fake.lexify('?????', letters='ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789'),
            'FLG_MULTIPLE_TITULACION': fake.random_element(elements=[0, 1]),
            'ID_TITULO_PREVIO_MASTER': fake.random_int(min=1, max=1000),
            'ID_PLAN_ESTUDIO_NK': fake.random_int(min=1, max=1000),
            'CURSO_IMPARTICION_ASIG': str(fake.random_int(min=1, max=99)).zfill(2),
            'ID_PERSONA_NIP_NK': fake.random_int(min=1, max=1000),
            'NUM_REGS_GRANO_MOV': fake.random_int(min=1, max=1000),
            'SUPER_EXPEDIENTE': fake.random_int(min=1, max=1000),
            'ID_PAIS_UNIVERSIDAD_ORIGEN_NK': fake.lexify('?????', letters='ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789'),
            'SN_CAMBIO_CICLO_GRADO': fake.random_element(elements=['S', 'N']),
            'FLG_EN_PROGRAMA_MOVILIDAD': fake.random_element(elements=[0, 1]),
            'ID_MODALIDAD_PLAN': fake.random_int(min=1, max=1000),
            'SN_INTERUNIVERSITARIO': fake.random_element(elements=['S', 'N']),
...
def generate_f_rendimiento(n=20000, dimension_dfs=None):
    ...
    for i in tqdm(range(1, n+1), desc="Generando rendimientos"):
        ...
        data.append({
            ...
            'SN_DOCTORADO_VIGENTE': fake.random_element(elements=['S', 'N']),
            'ID_DEDICACION': fake.random_int(min=1, max=1000),
            'FLG_COMPENSADA': fake.random_element(elements=[0, 1]),
            'FLG_TRANSFERIDA': fake.random_element(elements=[0, 1]),
            'ID_RANGO_NOTA_ADMISION': round(fake.random.uniform(0, 10), 2),
            'ID_PLAN_GRUPO_MATRICULA_NK': fake.random_int(min=1, max=1000),
            'ID_CENTRO_NK': fake.random_int(min=1, max=1000),
            'ID_RAMA_CONOCIMIENTO': fake.random_int(min=1, max=1000),
            'CURSO_MAS_ALTO_MATRICULADO': fake.random_int(min=1, max=1000),
            'CURSO_IMPARTICION_ASIG': str(fake.random_int(min=1, max=99)).zfill(2),
            'ORDEN_BOE_PLAN': fake.text(max_nb_chars=16).replace('\n', ' '),
            'FLG_MOVILIDAD_IN': fake.random_element(elements=[0, 1]),
            'ID_PLAN_ESTUDIO_NK': fake.random_int(min=1, max=1000),
            'FLG_ULTIMA_CONVOCATORIA': fake.random_element(elements=[0, 1]),
            'ID_CENTRO_IMPARTICION': fake.random_int(min=1, max=1000),
            'FLG_MATRICULADA': fake.random_element(elements=[0, 1]),
            'FLG_SUPERADA': fake.random_element(elements=[0, 1]),
            'ID_PLAN_GRUPO_MATRICULA': fake.random_int(min=1, max=1000),
            'ID_ESTUDIO_NK': fake.random_int(min=1, max=1000),
            'NOTA_ADMISION': round(fake.random.uniform(0, 10), 2),
            'SN_MOVILIDAD_IN': fake.random_element(elements=['S', 'N']),
            'FLG_RECONOCIDA': fake.random_element(elements=[0, 1]),
            'ID_FECHA_CARGA': int(generate_random_date(2020, 2023).replace('-', '')),
            'ID_TIPO_ESTUDIO': fake.random_int(min=1, max=1000),
            'SN_PROG_MOVILIDAD': fake.random_element(elements=['S', 'N']),
            'ID_CAMPUS_CENTRO': fake.random_int(min=1, max=1000),
            ...
            'ID_CENTRO_IMPARTICION_NK': fake.random_int(min=1, max=1000),
            'ID_POBLACION_CENTRO': fake.random_int(min=1, max=1000),
            'SN_COMPENSADA': fake.random_element(elements=['S', 'N']),
            'FLG_SUSPENDIDA': fake.random_element(elements=[0, 1]),
            'ID_EDAD_EST': fake.random_int(min=1, max=1000),
            'FLG_CALCULO_TASAS': fake.random_element(elements=[0, 1]),
            'ID_PAIS_FAMILIAR': fake.random_int(min=1, max=1000),
            'FLG_PRESENTADA': fake.random_element(elements=[0, 1]),
            'SN_CAMBIO_CICLO_GRADO': fake.random_element(elements=['S', 'N']),
            'FLG_EN_PROGRAMA_MOVILIDAD': fake.random_element(elements=[0, 1]),
            'ID_ASIGNATURA_NK': fake.random_int(min=1, max=1000),
            'ID_RANGO_NOTA_EGRACONS': round(fake.random.uniform(0, 10), 2),
            'ID_TIPO_ASIGNATURA': fake.random_int(min=1, max=1000),
            'ID_PAIS_FAMILIAR_NK': fake.lexify('?????', letters='ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789'),
