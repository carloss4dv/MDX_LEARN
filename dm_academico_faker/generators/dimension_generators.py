...
def generate_d_tiempo(start_year=2010, end_year=2023):
    """Genera datos para la tabla D_TIEMPO con relaciones completas entre niveles de tiempo"""
    data = []
    ...
    while current_date <= end_date:
        ...
        # Determinar día de la semana y si es laborable
        dia_semana = current_date.weekday()  # 0 = lunes, 6 = domingo
        es_laborable = 1 if dia_semana < 5 else 0  # Lunes a viernes son laborables
        
        # Determinar si es festivo nacional (simplificado)
        es_festivo = 0
        if (mes == 1 and dia == 1) or (mes == 12 and dia == 25):  # Año Nuevo y Navidad
            es_festivo = 1
        
        # Crear registro para el día
        data.append({
            'ID_DIA': id_dia,
            'NOMBRE_DIA': current_date.strftime('%d/%m/%Y'),
            'FECHA': current_date,
            'DIA_SEMANA_NUM': dia_semana + 1,  # 1 = lunes, 7 = domingo
            'DIA_SEMANA_NOMBRE': ['Lunes', 'Martes', 'Miércoles', 'Jueves', 'Viernes', 'Sábado', 'Domingo'][dia_semana],
            'DIA_MES': dia,
            ...
            'NOMBRE_TRIMESTRE': f"T{trimestre} {anyo}",
            'NUMERO_TRIMESTRE': trimestre,
            'ID_ANYO': id_anyo_actual,
            'NOMBRE_ANYO': str(anyo),
            'ANYO': anyo,
            # Campos adicionales para relaciones académicas
            'CURSO_ACADEMICO': f"{anyo}/{anyo+1}" if mes >= 9 else f"{anyo-1}/{anyo}",
            'PERIODO_LECTIVO': 'Primero' if 9 <= mes <= 12 or mes == 1 else 'Segundo' if 2 <= mes <= 6 else 'No lectivo',
            'ES_PERIODO_EXAMENES': 1 if mes in [1, 6] else 0
        })
...
def generate_d_tipo_estudio(n=10):
    ...
    Contiene los tipos de estudios oficiales y propios del sistema universitario.
    
    Args:
        n (int): Número de tipos de estudio a generar (máx. 10)
    ...
    tipos_estudio = [
        # nombre, NK, descripción, nivel ISCED, orden
        ('Grado', 1, 'Estudios oficiales de primer ciclo universitario (240 ECTS)', 6, 10),
        ('Máster Universitario', 2, 'Estudios oficiales de segundo ciclo universitario (60-120 ECTS)', 7, 20),
        ('Doctorado', 3, 'Estudios oficiales de tercer ciclo universitario', 8, 30),
        ('Título Propio', 4, 'Enseñanzas propias de la universidad', 7, 40),
        ('Curso de Adaptación', 5, 'Curso de adaptación a Grado', 6, 15),
        ('Diplomatura', 6, 'Estudios oficiales de primer ciclo (plan antiguo)', 5, 5),
        ('Licenciatura', 7, 'Estudios oficiales de primer y segundo ciclo (plan antiguo)', 6, 3),
        ('Ingeniería', 8, 'Estudios oficiales técnicos superiores (plan antiguo)', 6, 4),
        ('Ingeniería Técnica', 9, 'Estudios oficiales técnicos de primer ciclo (plan antiguo)', 5, 2),
        ('Arquitectura', 10, 'Estudios oficiales de arquitectura (plan antiguo)', 6, 1)
    ]
    
    # Limitar al número solicitado
    tipos_estudio = tipos_estudio[:n]
    
    for i, (nombre, nk, descripcion, nivel_isced, orden) in enumerate(tipos_estudio, 1):
        data.append({
            'ID_TIPO_ESTUDIO': i,
            'NOMBRE_TIPO_ESTUDIO': nombre,
            'ID_TIPO_ESTUDIO_NK': nk,
            'ID_TIPO_ESTUDIO_DESCR': i,
            'ORD_TIPO_ESTUDIO': nombre.upper(),
            'DESCRIPCION_TIPO_ESTUDIO': descripcion,
            'NIVEL_ISCED': nivel_isced,
            'FLG_OFICIAL': 1 if nombre not in ['Título Propio'] else 0,
            'FLG_PLAN_ANTIGUO': 1 if nombre in ['Diplomatura', 'Licenciatura', 'Ingeniería', 'Ingeniería Técnica', 'Arquitectura'] else 0,
            'FECHA_CARGA': generate_random_date(2015, 2020)
        })
    
    return pd.DataFrame(data)

def generate_d_rama_conocimiento(n=10):
    """Genera datos para la tabla D_RAMA_CONOCIMIENTO"""
    data = []
    ...
    ramas = [
        ...
        ('F', 'Multidisciplinar'),
        ('G', 'Ciencias Experimentales'),
        ('H', 'Enseñanzas Técnicas'),
        ('I', 'Humanidades'),
        ('J', 'Ciencias Sociales y Jurídicas')
    ]
    
    for i, (id_nk, nombre) in enumerate(ramas, 1):
        data.append({
            'ID_RAMA_CONOCIMIENTO': i,
            'NOMBRE_RAMA_CONOCIMIENTO': nombre,
            'ID_RAMA_CONOCIMIENTO_NK': id_nk
        })
...
def generate_d_convocatoria(n=5):
    ...
    for i, (id_nk, nombre, descripcion, orden, mes) in enumerate(convocatorias, 1):
        ...
        data.append({
            ...
            'FECHA_CARGA': generate_random_date(2020, 2023),
            'ID_CONVOCATORIA_NK': id_nk,
            'DESCRIPCION_CONVOCATORIA': descripcion,
            'ORDEN_CONVOCATORIA': orden,
            'MES_CONVOCATORIA': mes,
...
def generate_d_tipo_acceso_preinscripcion(n=15):
    ...
    for i, (id_nk, nombre) in enumerate(tipos_acceso, 1):
        data.append({
            ...
            'FECHA_CARGA': generate_random_date(2020, 2023),
            'ID_TIPO_ACCESO_PREINS_NK': id_nk
        })
...
def generate_d_edad_est(n=10):
    ...
    for i, (id_edad_est_nk, nombre_edad_est, descripcion) in enumerate(rangos_edad, 1):
        data.append({
            ...
            'DESCRIPCION_EDAD_EST': descripcion,
            'EDAD_MINIMA': int(id_edad_est_nk.split('-')[0]) if '-' in id_edad_est_nk and id_edad_est_nk[0].isdigit() else None,
            'EDAD_MAXIMA': int(id_edad_est_nk.split('-')[1]) if '-' in id_edad_est_nk and id_edad_est_nk.split('-')[1].isdigit() else None
        })
...
def generate_d_dedicacion(n=5):
    ...
    for i, (id_nk, nombre, descripcion, orden, porcentaje, horas, completa, parcial) in enumerate(dedicaciones[:n], 1):
        fecha_carga = generate_random_date(2018, 2023)
        fecha_vigencia = generate_random_date(2023, 2025)
        
        data.append({
            'ID_DEDICACION': i,
            'ID_DEDICACION_NK': id_nk,
            'NOMBRE_DEDICACION': nombre,
            'DESCRIPCION_DEDICACION': descripcion,
            'ORDEN_VISUALIZACION': orden,
            'PORCENTAJE_DEDICACION': porcentaje,
            'HORAS_SEMANALES': horas,
            'FECHA_CARGA': fecha_carga,
            'FECHA_FIN_VIGENCIA': fecha_vigencia,
            'SN_TIEMPO_COMPLETO': completa,
            'SN_TIEMPO_PARCIAL': parcial,
            'FLG_TIEMPO_COMPLETO': 1 if completa == 'S' else 0,
...
def generate_d_estado_adjudicacion(n=7):
    ...
    for i, (id_nk, nombre) in enumerate(estados, 1):
        data.append({
            ...
            'FECHA_CARGA': generate_random_date(2020, 2023),
            'ID_ESTADO_ADJUDICACION_NK': id_nk
        })
...
def generate_d_rango_nota_egracons(n=10):
    ...
    for i, (id_nk, nombre, nota_ref, limite_sup, limite_inf, desc, equiv_ects, equiv_esp, codigo_tipo) in enumerate(rangos, 1):
        ...
        data.append({
            ...
            'DESCRIPCION_EGRACONS': desc,
            'EQUIVALENCIA_ECTS': equiv_ects,
            'EQUIVALENCIA_ESP': equiv_esp,
...
def generate_d_centro_destino(n=50, universidad_df=None):
    ...
    for i in range(1, n+1):
        ...
        data.append({
            ...
            'ID_UNIVERSIDAD_NK': universidad['ID_UNIVERSIDAD_NK']
        })
    ...
    if centro_destino_df is not None:
        ...
        for _, row in centro_destino_df.iterrows():
            data.append({
                ...
                'ID_UNIVERSIDAD_NK': row['ID_UNIVERSIDAD_NK'],
...
def generate_d_estudio(n=100):
    ...
    for i in range(1, n+1):
        ...
        fecha_creacion = generate_random_date(1990, 2020)
        fecha_carga = generate_random_date(2020, 2023)
        ...
        data.append({
            ...
            'ES_INTERUNIVERSITARIO': 1 if random.random() < 0.15 else 0
        })
...
def generate_d_estudio_jerarq(n=100, plan_estudio_df=None, estudio_df=None, tipo_estudio_df=None, rama_conocimiento_df=None):
    ...
    if rama_conocimiento_df is None:
        rama_conocimiento_df = generate_d_rama_conocimiento(n=5)
...
def generate_d_proyecto_investigacion(n=30):
    ...
    for i in range(1, n+1):
        ...
        data.append({
            ...
            'TIPO_PROYECTO': descripcion_prefijo
        })
...
def generate_d_estado_solicitud_doctorado(n=10):
    ...
    for i, (codigo, nombre, descripcion, es_aprobado, es_rechazado, requiere_accion) in enumerate(estados, 1):
        data.append({
            ...
            'FLG_ESTADO_APROBADO': es_aprobado,
            'FLG_ESTADO_RECHAZADO': es_rechazado,
            'FLG_REQUIERE_ACCION': requiere_accion,
...
def generate_d_categoria_cuerpo_pdi(n=15):
    ...
    for i, (id_nk, nombre, tipo_contrato, grupo, doctor, sexenios, venia, orden) in enumerate(categorias[:n], 1):
        ...
        data.append({
            ...
            'ID_CATEGORIA_CUERPO_ESCALA_NK': id_nk,
            'TIPO_CONTRATO': tipo_contrato,
            'GRUPO_PROFESIONAL': grupo,
            'FLG_REQUIERE_DOCTORADO': 1 if doctor else 0,
            'SN_REQUIERE_DOCTORADO': 'S' if doctor else 'N',
            'FLG_PUEDE_SEXENIOS': 1 if sexenios else 0,
            'SN_PUEDE_SEXENIOS': 'S' if sexenios else 'N',
            'FLG_VENIA_DOCENDI': 1 if venia else 0,
            'SN_VENIA_DOCENDI': 'S' if venia else 'N',
...
def generate_d_tipo_asignatura(n=10):
    ...
    for i, (id_nk, codigo, nombre, descripcion, id_descr, orden, basic, obligatoria, proyecto) in enumerate(tipos_asignatura[:n], 1):
        ...
        data.append({
            ...
            'CODIGO_TIPO_ASIGNATURA': codigo,
            'DESCRIPCION_TIPO_ASIGNATURA': descripcion,
            'FECHA_CARGA': fecha_carga,
            'ORDEN_VISUALIZACION': orden,
            'SN_FORMACION_BASICA': basic,
            'SN_COMPUTO_ACADEMICO': obligatoria,
            'SN_PROYECTO_FIN_CARRERA': proyecto,
            'FLG_FORMACION_BASICA': 1 if basic == 'S' else 0,
            'FLG_COMPUTO_ACADEMICO': 1 if obligatoria == 'S' else 0,
            'FLG_PROYECTO_FIN_CARRERA': 1 if proyecto == 'S' else 0
        })
    
    return pd.DataFrame(data)

def generate_d_modalidad_asignatura(n=8):
    """Genera datos para la tabla D_MODALIDAD_ASIGNATURA
    
    Esta tabla contiene las diferentes modalidades de impartición de asignaturas.
    
    Args:
        n (int): Número de modalidades a generar
    ...
    modalidades = [
        (1, 'P', 'Presencial', 'Asignatura impartida 100% presencial'),
        (2, 'S', 'Semipresencial', 'Asignatura con partes presenciales y a distancia'),
        (3, 'D', 'A distancia', 'Asignatura impartida 100% a distancia'),
        (4, 'V', 'Virtual', 'Asignatura impartida en entorno virtual'),
        (5, 'H', 'Híbrida', 'Asignatura con modalidad híbrida flexible'),
        (6, 'I', 'Invertida', 'Asignatura con metodología de aula invertida'),
        (7, 'B', 'Bimodal', 'Asignatura con doble vía de seguimiento'),
        (8, 'E', 'Exterior', 'Asignatura impartida en entidades externas')
    ]
    
    # Limitar al número solicitado
    modalidades = modalidades[:n]
    
    for i, (id_nk, codigo, nombre, descripcion) in enumerate(modalidades, 1):
        data.append({
            'ID_MODALIDAD_ASIGNATURA': i,
            'ID_MODALIDAD_ASIGNATURA_NK': id_nk,
            'CODIGO_MODALIDAD': codigo,
            'NOMBRE_MODALIDAD_ASIGNATURA': nombre,
            'DESCRIPCION_MODALIDAD': descripcion,
            'ES_PRESENCIAL': 1 if codigo in ['P', 'S', 'H', 'I', 'B'] else 0,
            'ES_ONLINE': 1 if codigo in ['D', 'V', 'S', 'H', 'B'] else 0,
            'FECHA_CARGA': generate_random_date(2020, 2023)
        })
...
def generate_d_tipo_centro(n=8):
    ...
    for i, (id_nk, nombre, descripcion, orden, docencia, titulacion) in enumerate(tipos_centro[:n], 1):
        ...
        data.append({
            ...
            'ID_TIPO_CENTRO_NK': id_nk,
            'DESCRIPCION_TIPO_CENTRO': descripcion,
            'ORDEN_TIPO_CENTRO': orden,
            'FLG_DOCENCIA': 1 if docencia else 0,
            'SN_DOCENCIA': 'S' if docencia else 'N',
            'FLG_TITULACION_OFICIAL': 1 if titulacion else 0,
...
def generate_d_poblacion_centro(poblacion_df=None):
    ...
    for _, row in poblaciones_ext.iterrows():
        ...
        data.append({
            ...
            'NOMBRE_PROVINCIA': provincia if provincia else "Internacional",
            'ID_CCAA': row.get('ID_CCAA', 0),
            'NOMBRE_CCAA': region if region else "Internacional",
...
def generate_d_estado_credencial_acceso(n=7):
    """Genera datos para la tabla D_ESTADO_CREDENCIAL_ACCESO
    
    Esta dimensión contiene los diferentes estados que puede tener una credencial
    de acceso a la universidad (PAU, EBAU, etc.). Incluye información sobre la validez
    y el tipo de estado.
    
    Args:
        n (int): Número de estados a generar (máx. 7)
    ...
    estados = [
        {
            ...
            "descripcion": "La credencial ha sido recibida pero está pendiente de validación",
            "valido": False
        },
        {
            "id": 2,
            "codigo": "VALI",
            "nombre": "Validada",
            "descripcion": "La credencial ha sido validada correctamente",
            "valido": True
        },
        {
            "id": 3,
            "codigo": "RECH",
            "nombre": "Rechazada",
            "descripcion": "La credencial ha sido rechazada por no cumplir requisitos",
            "valido": False
        },
        {
            "id": 4,
            "codigo": "PROV",
            "nombre": "Provisional",
            "descripcion": "La credencial ha sido validada provisionalmente",
            "valido": True
        },
        {
            "id": 5,
            "codigo": "FALT",
            "nombre": "Faltan documentos",
            "descripcion": "No se puede validar porque faltan documentos",
            "valido": False
        },
        {
            "id": 6,
            "codigo": "EXPI",
            "nombre": "Expirada",
            "descripcion": "La credencial ha expirado y ya no es válida",
            "valido": False
        },
        {
            "id": 7,
            "codigo": "ANUL",
            "nombre": "Anulada",
            "descripcion": "La credencial ha sido anulada por la autoridad competente",
            "valido": False
        }
    ]
    ...
    for estado in estados:
        data.append({
            'ID_ESTADO_CREDENCIAL_ACCESO': estado["id"],
            'ID_ESTADO_CREDENCIAL_ACCESO_NK': estado["codigo"],
            'NOMBRE_ESTADO_CREDENCIAL_ACCESO': estado["nombre"],
            'DESCRIPCION_ESTADO_CREDENCIAL': estado["descripcion"],
            'SN_ESTADO_CREDENCIAL_VALIDO': 'S' if estado["valido"] else 'N',
            'ID_ESTADO_CREDENCIAL_ACCESO_DESCR': estado["id"]
        })
...
def generate_d_situacion_administrativa(n=10):
    ...
    for i, (id_nk, nombre, descripcion) in enumerate(situaciones, 1):
        data.append({
            ...
            'SN_ACTIVO': 'S' if id_nk == 'ACT' else 'N'
        })
...
def generate_d_area_conocimiento(n=10):
    """Genera datos para la tabla D_AREA_CONOCIMIENTO con todos los atributos completos
    
    Esta función genera datos completos para la tabla D_AREA_CONOCIMIENTO, incluyendo
    el código y nombre del área de conocimiento, así como metadatos adicionales.
    
    Args:
        n (int): Número de áreas de conocimiento a generar (máx. 10)
    ...
    areas_conocimiento = [
        ('MAT', 'Matemáticas', 'Ciencias exactas y formales', 1),
        ('FIS', 'Física', 'Ciencias naturales y experimentales', 2),
        ('QUI', 'Química', 'Ciencias naturales y experimentales', 3),
        ('BIO', 'Biología', 'Ciencias naturales y experimentales', 4),
        ('MED', 'Medicina', 'Ciencias de la salud', 5),
        ('INF', 'Informática', 'Ingeniería y tecnología', 6),
        ('DER', 'Derecho', 'Ciencias sociales y jurídicas', 7),
        ('ECO', 'Economía', 'Ciencias sociales y jurídicas', 8),
        ('FIL', 'Filología', 'Humanidades', 9),
        ('HIS', 'Historia', 'Humanidades', 10)
    ]
    
    for i, (id_nk, nombre, grupo, orden) in enumerate(areas_conocimiento[:n], 1):
        fecha_carga = generate_random_date(2018, 2023)
        
        data.append({
            'ID_AREA_CONOCIMIENTO': i,
            'ID_AREA_CONOCIMIENTO_NK': id_nk,
            'NOMBRE_AREA_CONOCIMIENTO': nombre,
            'GRUPO_AREA_CONOCIMIENTO': grupo,
            'ORDEN_VISUALIZACION': orden,
            'FECHA_CARGA': fecha_carga,
            'ES_ACTIVO': 1,
            'SN_ACTIVO': 'S'
...
def generate_d_dedicacion_alumno(n=5):
    """Genera datos para la tabla D_DEDICACION_ALUMNO
    
    Contiene los tipos de dedicación académica de los alumnos.
    ...
    dedicaciones = [
        # ID_NK, Nombre, ECTS mínimos, ECTS máximos, Descripción
        ('TC', 'Tiempo completo', 45, 90, 'Dedicación a tiempo completo'),
        ('TP', 'Tiempo parcial', 24, 45, 'Dedicación a tiempo parcial'),
        ('RM', 'Reducción por motivos laborales', 18, 30, 'Dedicación reducida por motivos laborales'),
        ('RF', 'Reducción por motivos familiares', 18, 30, 'Dedicación reducida por motivos familiares'),
        ('RD', 'Reducción por discapacidad', 12, 30, 'Dedicación reducida por discapacidad')
    ]
    
    # Limitar al número solicitado
    dedicaciones = dedicaciones[:n]
    
    for i, (id_nk, nombre, min_creditos, max_creditos, descripcion) in enumerate(dedicaciones, 1):
        data.append({
            'ID_DEDICACION_ALUMNO': i,
            'ID_DEDICACION_ALUMNO_NK': id_nk,
            'NOMBRE_DEDICACION_ALUMNO': nombre,
            'DESCRIPCION_DEDICACION': descripcion,
            'CREDITOS_MINIMOS': min_creditos,
            'CREDITOS_MAXIMOS': max_creditos,
            'FLG_DEFECTO': 1 if id_nk == 'TC' else 0,  # Tiempo completo como opción por defecto
            'FECHA_CARGA': generate_random_date(2015, 2023)
        })
