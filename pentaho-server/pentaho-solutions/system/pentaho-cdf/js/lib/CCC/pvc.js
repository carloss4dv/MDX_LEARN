...
define([ "./def", "./protovis-compat!", "./cdo", "jquery", "./tipsy" ], function(def, pv, cdo, $) {
    ...
    function cartAxis_labelDesiredAngles(value) {
        ...
        def.array.is(value) || (value = [ value ]);
        for (var angles = [], i = 0, ic = value.length; i !== ic; ++i) {
    ...
    pvc.brighterColorTransform = function(color) {
        return null == color ? null : (color.rgb ? color : pv.color(color)).brighter(.6);
    };
    ...
    def.type("pvc.LegendPanel", pvc.BasePanel).init(function(chart, parent, options) {
        ...
        _getTooltipFormatter: function(tipOptions) {
            ...
            return function(context) {
                ...
                return valueText !== context.pvMark.text() ? def.html.escape(valueText) : "";
            };
        }
    });
    ...
    pvc.BaseChart.add({
        ...
        _initMultiCharts: function() {
            ...
            smallDatas.length > multiOption("Max") && (chart._lastRenderMessage = {
        ...
        createSmallCharts: function() {
            ...
            if (multiInfo && (pageSmallCount = multiInfo.pageSmallCount)) {
                ...
                for (var childOptionsBase = this._buildSmallChartsBaseOptions(), ChildClass = chart.constructor, smallDatas = multiInfo.smallDatas, colCount = multiInfo.colCount, startIndex = multiInfo.pageStartIndex, index = 0; index < pageSmallCount; index++) {
                    var smallData = smallDatas[startIndex + index], colIndex = index % colCount, rowIndex = Math.floor(index / colCount), childOptions = def.set(Object.create(childOptionsBase), "smallColIndex", colIndex, "smallRowIndex", rowIndex, "title", smallData.absLabel, "titleVisible", !0, "data", smallData), smallChart = new ChildClass(childOptions);
                    if (coordRootAxesByScopeType) {
    ...
    def("pvc.visual.BasicSign", def.Object.extend({
        ...
        methods: {
            ...
            context: function(scene, createIndep) {
                ...
                if (null == state.cccContext) return state.cccContext = this._createContext(scene);
    ...
    def.type("pvc.AxisPanel", pvc.BasePanel).init(function(chart, parent, axis, options) {
        ...
        _calcDiscreteTicks: function() {
            ...
            var format, domainValues = axis.domainValues(), grouping = axis.role.grouping, tickFormatter = axis.option("TickFormatter");
            ...
            } else tickFormatter && (format = function(child, index) {
    ...
    def.type("pvc.PointAbstract", pvc.CategoricalAbstract).add({
        ...
        _getIsNullDatum: function() {
            var measureDimNames, continuousCategDimName, M, me = this;
            return function(datum) {
                if (!measureDimNames) {
                    measureDimNames = me.visualRoles.value.grouping.dimensionNames().slice();
                    var categRole = me.visualRoles.category;
                    categRole.isDiscrete() || (continuousCategDimName = categRole.grouping.singleDimensionName);
                    M = measureDimNames.length;
                }
                var atoms = datum.atoms;
                if (continuousCategDimName && null == atoms[continuousCategDimName].value) return !0;
                for (var i = 0; i < M; i++) if (null != atoms[measureDimNames[i]].value) return !1;
                return !0;
            };
        },
        defaults: {
